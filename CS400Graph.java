...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        ...
        if (!this.containsVertex(start))
            throw new NoSuchElementException("The start node does not exist");
        if (!this.containsVertex(end))
            throw new NoSuchElementException("The end node does not exist");
        ...
        // start of the path finding algorithm
        Vertex currentVertex = this.vertices.get(start);
        // add the current vertex into the visited hashtable
        visited.put(currentVertex.data, currentVertex);
        Path shortestPath = new Path(currentVertex);
        // add all the available edges into the frontier
        for (Edge edge : currentVertex.edgesLeaving){
            // construct an path through possible edges
            Path path = new Path(shortestPath, edge);
            // add all the paths to the frontier
            frontier.add(path);
        }

        // keep find the path if the target of the path is not the end
        while (!shortestPath.end.data.equals(end) && !frontier.isEmpty()){
            // get the head of the frontier
            shortestPath = frontier.remove();
            // update the current vertex
            currentVertex = shortestPath.end;
            // add the current vertex into the visited hashtable
            visited.put(currentVertex.data, currentVertex);

            // add all the available edges into teh frontier
            outerloop:
            for (Edge edge : currentVertex.edgesLeaving){
                // check if the target of the edge is already in the visited hashtable
                if (visited.containsKey(edge.target.data)){
                    continue;
                }

                // construct the path of current path
                Path path = new Path(shortestPath, edge);

                // check if the target of the edge is already in the frontier
                for (Path frontierPath : frontier){
                    if (frontierPath.end.equals(path.end)){
                        // if the length of the new path is shorter than the path
                        // found in the frontier
                        if (path.distance < frontierPath.distance){
                            // remove the old frontier, add the new path
        ...
        if (!shortestPath.end.data.equals(end)){
            throw new NoSuchElementException("The shortest path does not exist");
        }
